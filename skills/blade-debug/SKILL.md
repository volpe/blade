...
Apply the **blade spine**. Do not edit without a plausible root cause unless the user asks to explore freely.
...
4. **Evidence** — logs, bisect/blame, runtime state, minimal repro. When history/blast radius is unclear, parallel `scout`s (or explore) for independent legs (callers, related history, similar failures). **Budget mode:** 0–1 scout.
