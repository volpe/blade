...
Run after commit, before the concrete ship decision. Surface relevant results. This prepares missing approvals; existing authorization remains valid.
...
3. Resolve a missing required label before publication when:
   ...
   First use repository requirements and the actual change to select valid labels. Ask only when a material ambiguity remains; finish the PR draft before requesting that decision.
...
- Rebase only within recorded authority covering history rewriting. Resolve in-scope conflicts with evidence; request a decision when product or design intent is ambiguous. Never rewrite a shared/protected branch.
